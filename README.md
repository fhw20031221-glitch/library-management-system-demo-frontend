...
项目最初把 JWT Token 和用户信息保存在 `localStorage` 中。排查时发现，如果在同一个浏览器的不同标签页分别登录普通用户和管理员，后登录的账号会覆盖同一份 `library_token` 和 `library_user`，导致其他标签页刷新后也变成后登录账号，看起来像是账号“串味”。

原因是 `localStorage` 在同一个浏览器、同一个站点下的所有标签页之间共享。这个项目为了便于同时测试管理员和普通用户，已改为使用 `sessionStorage` 保存登录态。

当前效果：

- 同一个标签页刷新页面后，登录状态仍会保留。
- 不同标签页之间的 Token 相互隔离，可以分别登录不同账号。
- 后端 JWT 的生成、校验和权限判断方式没有变化，请求时仍通过 `Authorization: Bearer <token>` 携带 Token。
